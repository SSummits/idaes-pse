...
class MEAColumnData(PackedColumnData):
    ...
    def build(self):
        ...
        self.flood_fraction_max = Var(initialize=0.9,
                                      doc="Maximum value for flooding fraction")
        self.flood_fraction_max.fix()
        @self.Constraint(
            self.flowsheet().time,
            self.vapor_phase.length_domain
            )
        def flood_fraction_max_eqn(blk, t, x):
            return blk.flood_fraction[t, x] <= blk.flood_fraction_max
        self.flood_fraction_max_eqn.deactivate()
        ...
        def CV_Volume(blk, x):
            if x == 0:
                ...
                CV_length = (x-blk.liquid_phase.length_domain.prev(x)) * blk.length_column
    ...
    def build_internal_HE_model(self, mode, steam_temp=423.15):
        ...
        @self.Expression(
            self.flowsheet().time,
            self.vapor_phase.length_domain,
            doc="Heat exchanged to liquid phase")
    ...
    def activate_internal_HE(self, mode):
        ...
        self.eps_ref.unfix()
        self.eps_util.unfix()
        self.specific_area_util.unfix()
        self.N.unfix()
        
        self.N_start.unfix()
    
    def deactivate_internal_HE(self):
        self.eps_util.fix(0)
        self.specific_area_util.fix(0)
